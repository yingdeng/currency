...
var app = angular.module('app', ['ngRoute', 'demo', 'inputAmount']);
...
inputAmount.directive('isNumber', function(){
	return {
		require: 'ngModel',
		link: function(scope){
            console.log(scope.amountNo.number);
            ...
            scope.updateAmount = function() {
                scope.master = angular.copy(scope.amountNo.number);
            };
		}
	}
});
...
demo.directive('tradeBoard', function ($timeout) {
    return {
        ...
        link: function (scope, element, attrs, controller) {
            ...
            // Put Alert to the Model
            scope.alerts = [];
            scope.pushAlerts = function(action, type, value){
                scope.alerts.push({
                    action: action,
                    type: type,
                    value: value
            ...
            scope.bidAction = function () {
                console.log('Buy', scope.exchange.exchType, scope.bid);
                scope.pushAlerts('Buy', scope.exchange.exchType, scope.bid);
            };
           
            scope.ask = (Math.ceil((Math.random() * (scope.exchange.base) + 1) * 100)) / 100;
            scope.askAction = function () {
                console.log('Sell', scope.exchange.exchType, scope.ask);
                scope.pushAlerts('Sell', scope.exchange.exchType, scope.ask);
            };
